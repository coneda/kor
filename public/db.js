...
function indexBy(array, criteria, multiple=false) {
  let result = {};
...
Promise.all(promises).then(data => {
  lookup['kinds'] = indexBy(storage['kinds']['records'], 'id');
  lookup['collections'] = indexBy(storage['collections']['records'], 'id');

  lookup['authority_groups'] = indexBy(storage['authority_groups']['records'], 'id');
  lookup['authority_group_categories'] = indexBy(storage['authority_group_categories']['records'], 'id');
...
const paginate = (records, criteria) => {
  ...
  if (criteria['sort'] == 'created_at') {
    results = sortBy(results, e => e['created_at']);
