...
RSpec.describe RelationshipsController, :type => :controller do
  ...
  it "should list all relationships" do
    ...
    main_works = 10.times.map do
      FactoryGirl.create :artwork, :collection => default
    end
    side_works = 2.times.map do
      FactoryGirl.create :artwork, :collection => side
    end
    (main_works + side_works[0..0]).each do |e|
      Relationship.relate_and_save(main_artist, 'has created', e)
    end
    Relationship.relate_and_save(main_artist, 'is depicted by', side_works[1])
